...
#[allow(
    ...
    reason = "Windows implementation allocates"
)]
...
mod tests {
    ...
    fn unix_drive_aliases_translate_to_windows_roots() {
        assert_eq!(
            translate_unix_drive_path(Path::new("/c")).as_deref(),
            Some(Path::new("C:\\"))
        );
        assert_eq!(
    ...
    fn pattern_drive_alias_roots_require_forward_slash_prefix() {
        assert_eq!(
            pattern_drive_alias_root_impl(false, "", Some("d"), Some("logs")),
            None
        );
        assert_eq!(
        ...
        assert_eq!(
            pattern_drive_alias_root_impl(true, "", Some("mnt"), Some("data")),
            None
        );
    }
    ...
    fn non_drive_absolute_paths_are_left_native() {
        ...
        assert_eq!(
            translate_unix_drive_path(Path::new("/dev/null")).as_deref(),
            None
        );
        assert_eq!(
            translate_unix_drive_path(Path::new("/mnt/data")).as_deref(),
            None
        );
        assert_eq!(
            translate_unix_drive_path(Path::new("relative/path")).as_deref(),
            None
        );
        assert_eq!(
            translate_unix_drive_path(Path::new("\\d\\logs")).as_deref(),
            None
        );
        assert_eq!(
            translate_unix_drive_path(Path::new("\\mnt\\d\\logs")).as_deref(),
            None
        );
    }
}
