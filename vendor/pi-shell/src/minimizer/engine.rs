...
fn apply_identity(
    ...
    if filters::supports(&identity.program, subcommand) {
        ...
        let Ok(rust_output) = catch_unwind(AssertUnwindSafe(|| {
            filters::filter(&ctx, captured, exit_code)
        })) else {
...
fn apply_pipeline_overlay(
    ...
    let text = catch_unwind(AssertUnwindSafe(|| {
        pipeline.apply(&inner.text).into_owned()
    }))
...
static BUILTIN_PIPELINES: LazyLock<PipelineRegistry> = LazyLock::new(
    || match pipeline::parse_file(BUILTIN_FILTERS_TOML, "builtin") {
...
mod tests {
    ...
    fn config_from_settings(contents: &str) -> MinimizerConfig {
        ...
        let path = std::env::temp_dir().join(format!(
            "pi-shell-minimizer-engine-{}-{nonce}.toml",
            std::process::id()
        ));
...
"#,
    ...
    fn npx_def_does_not_overlay_routed_eslint() {
        ...
        for i in 0..120 {
            let _ = writeln!(
                input,
                "/src/file{i}.ts:{i}:1  error  Something is wrong  rule/name"
            );
        }
...
"#,
    ...
    fn segmented_chain_mode_is_only_for_eligible_safe_chains() {
        ...
        assert_eq!(
            mode_for("git diff ; printf done", &cfg),
            MinimizerMode::SegmentedChain
        );
    ...
    fn segmented_chain_supported_command_does_not_record_unknown() {
        ...
        assert!(
            !out.changed,
            "mixed chain must stay passthrough in whole-buffer minimization"
        );
    ...
    fn cpp_tools_minimize_through_dispatch() {
        ...
        assert!(should_minimize(
            "./build/foo_test --gtest_filter=Foo.*",
            &cfg
        ));
    ...
    fn git_status_chain_stays_opaque() {
        ...
        assert!(
            !out.changed,
            "same-subcommand status chain must stay passthrough"
        );
        assert_eq!(out.filter, "compound");
        assert_eq!(
            out.text, input,
            "captured output must be preserved verbatim"
        );
    ...
    fn git_commit_chain_differing_actions_stays_opaque() {
        ...
        assert!(
            !out.changed,
            "commit actions share a subcommand but not an output contract"
        );
        assert_eq!(out.filter, "compound");
        assert_eq!(out.text, input);
    }
    ...
    fn git_only_chain_differing_subcommands_stays_opaque() {
        ...
        assert!(
            !out.changed,
            "differing-subcommand git chain must stay passthrough"
        );
        assert_eq!(out.filter, "compound");
        assert_eq!(
            out.text, input,
    ...
    fn git_diff_same_raw_format_stays_opaque() {
        ...
        assert!(
            !out.changed,
            "same-raw-format diff chain must stay passthrough"
        );
        assert_eq!(out.filter, "compound");
        assert_eq!(
            out.text, input,
    ...
    fn mixed_chain_stays_opaque_in_whole_buffer_minimization() {
        ...
        let out = apply(
            "git status && echo IMPORTANT side-effect line",
            input,
            0,
            &cfg,
        );
    ...
    fn chains_with_exec_fd_mutation_are_not_segmented() {
        ...
        assert_eq!(
            mode_for("exec 2>err ; git status", &cfg),
            MinimizerMode::None
        );
        // The fd-mutating segment poisons the chain even when it is not first.
        assert_eq!(
            mode_for("git status ; exec >out", &cfg),
            MinimizerMode::None
        );
        // `exec` wrapped by `command`/`builtin` (with flags or env assignments)
        // mutates the same fds and must also block segmentation.
        assert_eq!(
            mode_for("command exec >out ; echo hi", &cfg),
            MinimizerMode::None
        );
        assert_eq!(
            mode_for("builtin exec >out ; echo hi", &cfg),
            MinimizerMode::None
        );
        assert_eq!(
            mode_for("git diff ; command -p exec 2>err", &cfg),
            MinimizerMode::None
        );
        assert_eq!(
            mode_for("FOO=\"a b\" exec >out ; echo hi", &cfg),
            MinimizerMode::None
        );
        assert_eq!(
            mode_for("FOO=\"a b\" command exec >out ; echo hi", &cfg),
            MinimizerMode::None
        );
        ...
        assert_eq!(
            mode_for("alias cat='printf hacked' ; cat file", &cfg),
            MinimizerMode::None
        );
        assert_eq!(
            mode_for("unalias cat ; cat file", &cfg),
            MinimizerMode::None
        );
        // A real command merely named with `exec` as an argument is not the
        // builtin and must NOT block segmentation.
        assert_eq!(
            mode_for("echo exec ; printf done", &cfg),
            MinimizerMode::SegmentedChain
        );
    ...
    fn rake_routes_regression_unminimized() {
        ...
        assert!(
            out.changed,
            "rake routes should be minimized by the rails-routes def"
        );
        assert!(!out.text.contains("Prefix"));
        assert!(out.text.contains("root GET"));
    }
    ...
    fn rails_db_migrate_keyword_in_name_not_dropped() {
        ...
        assert!(
            out.text.contains("CreateTestResults"),
            "CreateTestResults was dropped"
        );
        assert!(
            out.text.contains("AddIndexToOrders"),
    ...
    fn rails_def_does_not_overlay_routed_minitest() {
        ...
        assert!(
            rails.text.contains("Expected: true"),
            "rails test dropped minitest failure detail"
        );
        assert!(rails.text.contains("Actual: false"));
        assert!(
            rake.text.contains("Expected: true"),
            "rake test dropped minitest failure detail"
        );
    ...
    fn rails_routes_def_does_not_overlay_routed_minitest() {
        ...
        assert!(
            out.text.contains("Expected: true"),
            "rails test dropped minitest failure detail"
        );
    }
}
...
mod pipeline_integration_tests {
    ...
    fn gradle_routes_to_jvm_filter_via_apply() {
        ...
        assert!(
            out.changed,
            "gradle build filter strips task-progress noise"
        );
        assert!(out.text.contains("BUILD SUCCESSFUL"));
        assert!(
            !out.text.contains("> Task :"),
            "task lines stripped; got:\n{}",
            out.text
        );
        assert_eq!(
            out.filter, "gradle",
            "telemetry label routes through jvm program_label"
        );
    }
