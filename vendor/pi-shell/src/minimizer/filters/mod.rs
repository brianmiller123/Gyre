...
pub fn supports(program: &str, subcommand: Option<&str>) -> bool {
    match program {
        ...
        "npx" => {
            matches!(
                subcommand,
                Some("tsc" | "eslint" | "biome" | "jest" | "vitest" | "playwright")
        ...
        "uv" => {
            ...
            matches!(subcommand, Some("pytest" | "ruff" | "mypy" | "-m"))
                || pkg::supports(subcommand)
        }
...
mod tests {
    ...
    fn run_invoked_word_picks_script_not_arguments() {
        ...
        assert_eq!(
            run_invoked_word("npm run --silent test:unit"),
            Some("test:unit")
        );
    ...
    fn pkg_test_routing_ignores_test_as_argument() {
        ...
        // a test
        assert!(!is_pkg_test_invocation(&ctx(
            "npm",
            Some("run"),
            "npm run build -- test",
            &config
    ...
    fn uv_wrapper_ignores_tool_as_argument() {
        ...
        assert_eq!(
            uv_wrapper_tool(&ctx("uv", Some("run"), "uv run echo pytest", &config)),
            None
        );
        assert_eq!(
    ...
    fn uv_wrapper_skips_value_taking_option_values() {
        ...
        assert_eq!(
            uv_wrapper_tool(&ctx(
                "uv",
                Some("run"),
                "uv run --with foo python -m pytest",
                &config
            )),
            Some("pytest")
        );
    ...
    fn uv_run_python_script_with_pytest_argument_stays_opaque() {
        ...
        let context = ctx(
            "uv",
            Some("run"),
            "uv run python scripts/report.py -m pytest",
            &config,
        );
    ...
    fn normalize_uv_form_unit_pytest_subcommand() {
        assert_eq!(
            super::normalize_uv_form(Some("pytest"), "uv pytest"),
            Some("pytest")
        );
    }
    ...
    fn normalize_uv_form_unit_dash_m_pytest() {
        assert_eq!(
            super::normalize_uv_form(Some("-m"), "uv -m pytest tests/"),
            Some("pytest")
        );
    }
    ...
    fn normalize_uv_form_unit_unknown_returns_none() {
        assert_eq!(
            super::normalize_uv_form(Some("unknown"), "uv unknown"),
            None
        );
    ...
    fn bundle_wrapped_command_extracts_inner_command() {
        ...
        assert_eq!(
            super::bundle_wrapped_command("bundle exec rspec"),
