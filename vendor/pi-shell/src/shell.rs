        ...
        minimizer::engine::MinimizerMode::WholeCommand => {
            let Some(config) = options.minimizer.as_ref() else {
                return Err(Error::msg(
                    "Missing minimizer config for whole-command mode",
                ));
            };
            CommandCaptureMode::Buffered {
                max_capture_bytes: config.max_capture_bytes as usize,
            }
        }
...
fn pipe_to_files(label: &str) -> Result<(fs::File, fs::File)> {
    let (r, w) = os_pipe::pipe()
        .map_err(|err| Error::msg(format!("Failed to create {label} pipe: {err}")))?;
    ...
    let (r, w): (fs::File, fs::File) = {
        ...
        unsafe {
            (
                FromRawHandle::from_raw_handle(r),
                FromRawHandle::from_raw_handle(w),
            )
        }
    };
...
mod tests {
    ...
    async fn copy_env_skips_non_utf8_entries_and_merges_path() {
        ...
        assert!(
            value("GHOSTTY_BIN_DIR").is_none(),
            "non-UTF-8 value must be skipped"
        );
    ...
    async fn pgrep_matches_name_and_pkills_signal_probe() {
        ...
        assert!(
            still_running,
            "signal 0 must not terminate a matching process"
        );
    }
    ...
    async fn pkill_accepts_platform_signal_names() {
        ...
        assert!(
            still_running,
            "{signal} should be ignored by the test process"
        );
    }
    ...
    async fn pkill_interactive_prompt_honors_cancellation() {
        ...
        assert!(
            still_running,
            "cancelled pkill must not signal its pending target"
        );
    }
    ...
    async fn pgrep_requires_an_externally_locked_pidfile() {
        ...
        let command = format!(
            "pgrep -L -F {} -x {name}",
            quote_arg(pidfile.to_str().expect("utf8 pidfile"))
        );
        ...
        assert!(
            unlocked_output.contains("is not locked"),
            "{unlocked_output:?}"
        );
    }
    ...
    async fn pkill_signals_every_matching_process() {
        ...
        let statuses = time::timeout(Duration::from_secs(2), async {
        ...
        assert!(
            statuses.is_ok(),
            "pkill did not signal every matching process"
        );
    }
    ...
    async fn ps_builtin_supports_extended_format_specifiers() {
        ...
        #[cfg(unix)]
        {
            ...
            assert!(
                u64::from_str_radix(fields[6], 16).is_ok(),
                "flags: {:?}",
                fields[6]
            );
    ...
    async fn ps_builtin_accepts_tpgid_alongside_job_control_columns() {
        ...
        let pid = std::process::id();
        let (result, output) = execute_captured(format!(
            "ps -o pid,ppid,pgid,tpgid,sess,stat,tty,command -p {pid}"
        ))
        .await;
        assert_eq!(result.exit_code, Some(0), "{output:?}");
        let header = output.lines().next().unwrap_or_default();
        for label in [
            "PID", "PPID", "PGID", "TPGID", "SID", "STAT", "TTY", "COMMAND",
        ] {
    ...
    async fn kill_builtin_accepts_numeric_signal_for_multiple_processes() {
        ...
        let result = session
            .shell
            .run_string(
                format!("kill -9 {first_pid} {second_pid}"),
                &source_info,
                &params,
            )
            .await
        ...
        assert_eq!(
            first_status.expect("first wait").signal(),
            Some(libc::SIGKILL)
        );
        assert_eq!(
            second_status.expect("second wait").signal(),
            Some(libc::SIGKILL)
        );
    }
    ...
    async fn kill_builtin_defaults_to_sigterm() {
        ...
        assert_eq!(
            status.expect("checked timeout").expect("child wait").code(),
            Some(42)
        );
    }
    ...
    async fn kill_builtin_preserves_negative_pid_process_group_operands() {
        ...
        let result = session
            .shell
            .run_string(
                format!("kill -TERM -- -{group_pid} {plain_pid}"),
                &source_info,
                &params,
            )
            .await
    ...
    async fn kill_builtin_signals_group_when_marker_precedes_negative_pid() {
        ...
        assert_eq!(
            exit_code(&default_result),
            0,
            "`kill -- -<pgid>` should succeed"
        );
        assert_eq!(
            exit_code(&named_result),
            0,
            "`kill -s TERM -- -<pgid>` should succeed"
        );
    ...
    async fn kill_builtin_signals_every_process_in_a_jobspec_pipeline() {
        ...
        assert_eq!(
            exit_code(&killed),
            0,
            "`kill %1` should signal every pipeline process"
        );
        let _ = session
            .shell
        ...
        for pid in pids {
            assert!(
                process::Process::from_pid(pid).is_none(),
                "pipeline process {pid} survived"
            );
        }
    }
    ...
    async fn kill_builtin_refuses_own_pid() {
        ...
        if std::env::var_os(MARKER).is_none() {
            run_isolated_kill_test("shell::tests::kill_builtin_refuses_own_pid", MARKER, false)
                .await;
            return;
        }
        ...
        assert_eq!(
            probe.exit_code,
            Some(0),
            "signal 0 should probe the shell process"
        );
        let (result, output) = execute_captured(format!(
        ...
        assert_eq!(
            result.exit_code,
            Some(0),
            "the shell must survive a direct kill attempt"
        );
        assert!(
    ...
    async fn kill_builtin_refuses_own_process_group() {
        ...
        assert_eq!(
            result.exit_code,
            Some(0),
            "the shell must survive a group kill attempt"
        );
        assert!(
            output.contains("refusing to signal the shell process"),
            "{output:?}"
    ...
    async fn kill_builtin_refuses_ancestors_but_not_unrelated_processes() {
        ...
        assert!(
            output.contains("ancestor=1"),
            "the ancestor kill must report failure: {output:?}"
        );
        // The same guard must leave a process outside our ancestry alone, or `kill`
        // would be useless.
        assert!(
            output.contains("child=0"),
            "an unrelated child must remain signallable: {output:?}"
        );
    }
    ...
    async fn pkill_builtin_excludes_own_pid() {
        ...
        if std::env::var_os(MARKER).is_none() {
            run_isolated_kill_test(
                "shell::tests::pkill_builtin_excludes_own_pid",
                MARKER,
                false,
            )
            .await;
        ...
        assert_eq!(
            result.exit_code,
            Some(0),
            "the shell must survive an explicit pkill"
        );
        assert!(output.contains("survived"), "{output:?}");
    }
    ...
    async fn uutils_sort_compress_program_uses_shell_path_and_stderr() {
        ...
        assert_eq!(
            ...
            expected,
            "compressed external sort misordered"
        );
    ...
    async fn uutils_mkdir_resolves_cwd_and_displays_operand() {
        ...
        // host process cwd.
        assert!(
            tmp.join("a/b/c").is_dir(),
            "nested dirs not created under shell cwd"
        );
        assert!(tmp.join("rel").is_dir(), "rel not created under shell cwd");
        assert!(
            !std::path::Path::new("a/b/c").exists(),
            "mkdir leaked into process cwd"
        );
        ...
        assert!(
            out.contains("'rel'"),
            "verbose output missing operand `rel`: {out:?}"
        );
        assert!(
            !out.contains(tmp_str),
            "verbose output leaked absolute path: {out:?}"
        );
    ...
    async fn every_utility_builtin_is_registered_under_its_own_name() {
        ...
        for name in names {
            let exec = session
                .shell
                .run_string(
                    format!("PATH= {name} --help > help.txt 2> err.txt"),
                    &source_info,
                    &params,
                )
                .await
    ...
    async fn uutils_mkdir_expands_tilde_for_every_brace_element() {
        ...
        assert!(
            home.join("proj/a").is_dir(),
            "~/proj/a not created under HOME"
        );
        assert!(
            home.join("proj/b").is_dir(),
            "~/proj/b not created under HOME"
        );
        // The buggy path created a literal `~` tree in the shell cwd.
        assert!(!cwd.join("~").exists(), "literal ~ tree leaked into cwd");
        assert!(
            !cwd.join("a").exists(),
            "unexpanded element leaked into cwd"
        );
    ...
    async fn uutils_mkdir_help_and_bad_flag_do_not_exit_process() {
        ...
        assert!(
            help_text.contains("mkdir"),
            "help text missing util name: {help_text:?}"
        );
        ...
        assert!(
            !err_text.is_empty(),
            "usage error should be reported to stderr"
        );
    ...
    async fn uutils_head_streams_stdin_and_reads_files() {
        ...
        assert_eq!(
            std::fs::read_to_string(tmp.join("out_file.txt")).unwrap(),
            "l1\nl2\n"
        );
        ...
        assert_eq!(
            std::fs::read_to_string(tmp.join("out_pipe.txt")).unwrap(),
            "a\nb\n"
        );
        ...
        assert_eq!(
            std::fs::read_to_string(tmp.join("out_obs.txt")).unwrap(),
            "1\n"
        );
    ...
    async fn uutils_head_help_and_bad_flag_do_not_exit_process() {
        ...
        assert!(
            ...
            "help text not localized: {help_text:?}"
        );
    ...
    async fn uutils_filters_listing_find_grep() {
        ...
        let ls = read("ls.txt");
        assert!(
            ls.contains("data.txt") && ls.contains("sub"),
            "ls output: {ls:?}"
        );
    ...
    async fn rg_builtin_uses_ripgrep_defaults() {
        ...
        let out = read("rg.txt");
        assert!(
            out.contains("data.txt:needle"),
            "rg missed visible file: {out:?}"
        );
        assert!(
            out.contains("sub/nested.txt:needle"),
            "rg missed nested file: {out:?}"
        );
        assert!(
            !out.contains(".hidden.txt"),
            "rg searched hidden file by default: {out:?}"
        );
        assert!(
            !out.contains("ignored.log"),
            "rg ignored .gitignore by default: {out:?}"
        );
        assert!(
            !out.contains("binary.bin"),
            "rg printed binary file by default: {out:?}"
        );
        ...
        assert_eq!(
            exit_code(&explicit_binary),
            0,
            "explicit binary file should be searched"
        );
    ...
    async fn fd_builtin_uses_fd_defaults() {
        ...
        assert!(
            ...
            "fd missed visible file: {out:?}"
        );
        assert!(
            out.contains("sub/needle.rs"),
            "fd missed nested file: {out:?}"
        );
        assert!(
            !out.contains(".hidden-needle.txt"),
            "fd searched hidden file: {out:?}"
        );
        assert!(
            !out.contains("ignored-needle.log"),
            "fd ignored .gitignore: {out:?}"
        );
        assert!(
            !out.contains("fdignored-needle.tmp"),
            "fd ignored .fdignore: {out:?}"
        );
        assert!(
            !out.contains("excluded-needle.vcs"),
            "fd ignored .git/info/exclude: {out:?}"
        );
        ...
        assert!(
            unrestricted.contains(".hidden-needle.txt"),
            "-u should include hidden files"
        );
        assert!(
            unrestricted.contains("ignored-needle.log"),
            "-u should include gitignored files"
        );
        assert!(
            unrestricted.contains("fdignored-needle.tmp"),
            "-u should include fdignored files"
        );

        session
            .shell
            .run_string(
                "fd --no-ignore-vcs needle > no-ignore-vcs.txt",
                &si,
                &params,
            )
            .await
        ...
        assert_eq!(
            exit_code(&no_match),
            0,
            "ordinary fd no-match should still succeed"
        );
    ...
    async fn rg_builtin_defaults_to_cwd_unless_stdin_is_pipeline() {
        ...
        assert!(
            files.contains("data.txt"),
            "--files should list cwd files: {files:?}"
        );
        assert!(
            !files.contains("not-a-path"),
            "--files must not read piped stdin: {files:?}"
        );
    ...
    async fn grep_quiet_and_line_regexp() {
        ...
        session
            .shell
            .run_string(
                "grep -qx bar data.txt && echo XHIT > xhit.txt",
                &si,
                &params,
            )
            .await
        ...
        session
            .shell
            .run_string(
                "grep -qx ba data.txt || echo XMISS > xmiss.txt",
                &si,
                &params,
            )
            .await
    ...
    async fn uutils_rm_missing_file_prints_single_diagnostic() {
        ...
        assert_ne!(
            exit_code(&rm),
            0,
            "rm of a missing file must report failure"
        );
        let err = std::fs::read_to_string(tmp.join("err.txt")).expect("err.txt");
        ...
        assert_eq!(
            lines.len(),
            1,
            "rm should emit exactly one diagnostic line, got: {err:?}"
        );
        assert!(
            lines[0].contains("nonexistent.txt"),
    ...
    async fn uutils_find_display_and_actions_split_paths() {
        ...
        assert!(
            !tmp.join("sub/drop.tmp").exists(),
            "-delete should remove the matched file"
        );
        assert!(
            tmp.join("keep.log").exists(),
            "-delete must not touch unmatched files"
        );
    ...
    async fn uutils_jq_filters_json_and_propagates_exit_status() {
        ...
        session
            .shell
            .run_string(
                "printf 'null' | jq -e . > /dev/null; printf %s $? > code.txt",
                &si,
                &params,
            )
            .await
    ...
    async fn uutils_builtins_respect_disable_env() {
        ...
        assert!(
            default.shell.builtin_mut("head").is_some(),
            "head registered by default"
        );
        assert!(
            default.shell.builtin_mut("rg").is_some(),
            "rg registered by default"
        );
        assert!(
            default.shell.builtin_mut("rm").is_some(),
            "rm registered by default"
        );
        assert!(
            default.shell.builtin_mut("mv").is_some(),
            "mv registered by default"
        );
        ...
        let mut all_off = create_session(&session_with(&[("PI_DISABLE_UUTILS_BUILTINS", "1")]))
            ...
            .expect("create_session");
        assert!(
            all_off.shell.builtin_mut("head").is_none(),
            "kill-switch drops head"
        );
        assert!(
            all_off.shell.builtin_mut("rg").is_none(),
            "kill-switch drops rg"
        );
        assert!(
            all_off.shell.builtin_mut("rm").is_none(),
            "kill-switch drops rm"
        );
        ...
        let mut rm_off = create_session(&session_with(&[("PI_DISABLE_RM_BUILTIN", "1")]))
            ...
            .expect("create_session");
        assert!(
            rm_off.shell.builtin_mut("rm").is_none(),
            "rm disabled individually"
        );
        assert!(rm_off.shell.builtin_mut("mv").is_some(), "mv stays enabled");
        assert!(
            rm_off.shell.builtin_mut("head").is_some(),
            "head stays enabled"
        );
    }
...
"#,
    ...
    async fn segmented_false_and_printf_skips_second_and_returns_nonzero() {
        ...
        assert!(
            result.minimized.is_none(),
            "chain noop must not surface telemetry"
        );
    }
...
"#,
        ...
        assert!(
            result.minimized.is_none(),
            "short pwd output must not be minimized"
        );
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(output, expected);
    }
    ...
    async fn segmented_chain_with_redirect_executes_correctly() {
        ...
        assert!(
            ...
            "redirect must suppress segment-1 stdout"
        );
        assert_eq!(output, expected);
        let minimized = result.minimized.expect("long output should be minimized");
        assert_eq!(minimized.original_text, expected);
    ...
    async fn cancelling_one_run_spares_a_concurrent_runs_child() {
        ...
        assert_eq!(
            b_ready.trim(),
            "ready",
            "run B should reach its long sleep before run A starts"
        );
        ...
        assert!(
            a_child_pid > 0,
            "got non-positive run A child pid: {a_child_pid}"
        );
        ...
        assert_eq!(
            result_a.exit_code,
            Some(0),
            "cancelling run B must not SIGTERM run A's child"
        );
    ...
    async fn cancelling_while_sourcing_snapshot_kills_snapshot_foreground_child() {
        ...
        assert!(
            result.cancelled,
            "cancelling while sourcing a snapshot should report cancellation"
        );
        assert_eq!(
    ...
    async fn nohup_background_captures_operand_pid() {
        ...
        assert!(
            pid.parse::<i32>().is_ok_and(|pid| pid > 0),
            "invalid PID output: {out:?}"
        );
    }
    ...
    async fn streaming_output_backpressures_on_bounded_channel_without_loss() {
        ...
        assert_eq!(
            ...
            "streamed bytes were dropped under backpressure"
        );
    }
}
