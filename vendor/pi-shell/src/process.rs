...
mod platform {
    ...
    impl Process {
        ...
        fn live_bsdinfo(&self) -> Option<libc::proc_bsdinfo> {
            ...
            if info.pbi_start_tvsec == self.start_tvsec
                && info.pbi_start_tvusec == self.start_tvusec
            {
                Some(info)
            } else {
                None
    ...
    pub fn find_by_path(target: &str) -> Vec<Process> {
        ...
        loop {
            ...
            if let Some(handle) = open_process(pid, PROCESS_QUERY_LIMITED_INFORMATION) {
                ...
                let ok = unsafe {
                    QueryFullProcessImageNameW(handle.as_raw(), 0, buf.as_mut_ptr(), &raw mut size)
                        != 0
                };
                if ok {
                    ...
                    if path == target
                        && let Some(process) =
                            Process::from_pid(i32::try_from(pid).unwrap_or_default())
                    {
                        matches.push(process);
                    }
                }
...
async fn wait_for_exit(
    ...
    while timeout.is_none_or(|limit| elapsed < limit) {
        let sleep_for = timeout.map_or(poll_interval, |limit| {
            limit.saturating_sub(elapsed).min(poll_interval)
        });
...
#[allow(
    ...
    reason = "Dispatches to platform-specific implementation"
)]
#[must_use]
pub fn kill_process_group(pgid: i32, signal: i32) -> bool {
...
mod tests {
    ...
    fn add_process_bypasses_from_pid_lookup() {
        ...
        assert!(
            !targets.is_empty(),
            "add_process must record the pinned handle"
        );
