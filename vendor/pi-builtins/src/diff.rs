...
fn diff_main(files: &[OsString], opts: Options<'_>, host: &mut Host) -> Result<i32, String> {
    ...
    let differed = if let (Operand::Dir(res_a), Operand::Dir(res_b)) = (&op_a, &op_b) {
        ...
        diff_pair(
            &name_a, &bytes_a, mtime_a, &name_b, &bytes_b, mtime_b, opts, None, host,
        )?
    };
...
impl<'a> FileLines<'a> {
    fn split(text: &'a str) -> Self {
        ...
        FileLines {
            lines,
            missing_newline: !text.is_empty() && !text.ends_with('\n'),
        }
    }
}
...
fn diff_pair(
    ...
    if bytes_a == bytes_b {
        if opts.report_identical {
            wline(
                host,
                format_args!("Files {label_a} and {label_b} are identical"),
            )?;
        }
        return Ok(false);
    }
    if is_binary(bytes_a) || is_binary(bytes_b) {
        if opts.brief {
            ...
            wline(
                host,
                format_args!("Binary files {label_a} and {label_b} differ"),
            )?;
        }
        return Ok(true);
    }
    ...
    match opts.format {
        ...
        Format::Unified(context) => {
            wline(
                host,
                format_args!("--- {}", header(opts.labels.first(), name_a, mtime_a)),
            )?;
            wline(
                host,
                format_args!("+++ {}", header(opts.labels.get(1), name_b, mtime_b)),
            )?;
    ...
    for (op, &sup) in ops.iter().zip(suppressed) {
        ...
        match op.tag() {
            DiffTag::Delete => {
                ...
                wline(
                    host,
                    format_args!("{}d{}", normal_range(&old_range), new_range.start),
                )?;
            ...
            DiffTag::Insert => {
                wline(
                    host,
                    format_args!("{}a{}", old_range.start, normal_range(&new_range)),
                )?;
...
fn write_context_format(
    ...
    for (first, last) in group_ops(ops, suppressed, context) {
        ...
        wline(
            host,
            format_args!("*** {} ****", context_range(old_start, old_count)),
        )?;
        ...
        wline(
            host,
            format_args!("--- {} ----", context_range(new_start, new_count)),
        )?;
        if group
            .iter()
...
mod tests {
    ...
    fn unified_flag_emits_unified_with_timestamped_headers() {
        ...
        assert_eq!(
            (stamp.as_bytes()[4], stamp.as_bytes()[7]),
            (b'-', b'-'),
            "got: {stamp}"
        );
        assert!(stamp.contains('.') && (stamp.contains(" +") || stamp.contains(" -")));
        assert!(
            stdout.contains("@@ -1,3 +1,3 @@\n one\n-two\n+TWO\n three\n"),
            "got: {stdout}"
        );
    }
    ...
    fn three_labels_are_rejected() {
        ...
        let (code, _, stderr) = run_in(
            dir.path(),
            "",
            &["-L", "1", "-L", "2", "-L", "3", "a.txt", "b.txt"],
        );
        assert_eq!(code, 2);
        assert!(
            stderr.contains("too many file label options"),
            "got: {stderr}"
        );
    }
