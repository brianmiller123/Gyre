                ...
                for arg in &self.args {
                    match *arg {
                        ...
                        Arg::FileArg(ref parts) => {
                            command.arg(parts.join(path_to_file.as_os_str()))
                        }
                    };
                }
        ...
        fn get_time(matcher_io: &mut MatcherIO, today_start: bool) -> SystemTime {
            if today_start {
                ...
                let duration_since_unix_epoch =
                    matcher_io.now().duration_since(UNIX_EPOCH).unwrap();
                let seconds_since_unix_epoch = duration_since_unix_epoch.as_secs();
                let utc_time =
                    DateTime::from_timestamp(seconds_since_unix_epoch as i64, 0).unwrap();
                    ...
                    match parse_str_to_newer_args(args[i]) {
                        Some((x_option, y_option)) => {
                            ...
                            if y_option == "t" {
                                ...
                                let Some(comparable_time) = parse_date_str_to_timestamps(time)
                                else {
                                    return Err(From::from(format!(
                                ...
                                i += 1;
                                Some(
                                    NewerTimeMatcher::new(newer_time_type, comparable_time)
                                        .into_box(),
                                )
                            } else {
...
impl StandardDependencies {
    ...
    fn new(host: &Host) -> Self {
        Self {
            output: Rc::new(RefCell::new(host.stdout_clone())),
            now: SystemTime::now(),
        }
    }
}
...
fn apply_find_entry(
    ...
    (
        matcher_io.should_quit(),
        matcher_io.should_skip_current_dir(),
    )
}
...
fn rewrite_bsd_invocation(args: &[&str], host: &mut Host) -> Option<Vec<String>> {
    ...
    rewritten.splice(
        i..i,
        ["-regextype".to_string(), "posix-extended".to_string()],
    );
    Some(rewritten)
}
...
mod tests {
    ...
    fn type_accepts_comma_separated_list() {
        ...
        let (code, capture) = run(
            &root,
            &[root.display().to_string(), "-type".into(), "f,d".into()],
        );
        assert_eq!(code, 0, "stderr: {}", capture.err());
        assert_eq!(capture.err(), "");
        let mut matches: Vec<PathBuf> = capture.out().lines().map(PathBuf::from).collect();
    ...
    fn type_w_parses_and_matches_nothing() {
        ...
        let (code, capture) = run(
            &root,
            &[root.display().to_string(), "-type".into(), "w".into()],
        );
        assert_eq!(code, 0, "stderr: {}", capture.err());
        assert_eq!(capture.err(), "");
        assert_eq!(capture.out(), "");
    ...
    fn regex_matches_whole_path_only() {
        ...
        let (code, capture) = run(
            &root,
            &[root.display().to_string(), "-regex".into(), r"c\.rs".into()],
        );
        assert_eq!(code, 0, "stderr: {}", capture.err());
        assert_eq!(capture.out(), "");
    }
    ...
    fn size_accepts_t_and_p_suffixes() {
        ...
        assert_eq!(
            matches,
            vec![root.join("a.txt"), root.join("b.md"), root.join("c.rs")]
        );

        let (code, capture) = run(
            &root,
            &[root.display().to_string(), "-size".into(), "+1P".into()],
        );
        assert_eq!(code, 0, "stderr: {}", capture.err());
        assert_eq!(capture.err(), "");
        assert_eq!(capture.out(), "");
