...
mod buffer_hint {
    ...
    mod tests {
        ...
        fn desired_buffer_matches_total_when_small() {
            ...
            let expected = ((six_mebibytes as u128) * 12).clamp(
                six_mebibytes as u128,
...
mod check {
    ...
    pub fn check(path: &OsStr, settings: &GlobalSettings) -> SortResult<()> {
        ...
        for _ in 0..2 {
            let _ =
                recycled_sender.send(RecycledChunk::new(if settings.buffer_size < 100 * 1024 {
        ...
        while let Ok(chunk) = loaded_receiver.recv() {
            ...
            for (a, b) in chunk.lines().iter().tuple_windows() {
                line_idx += 1;
                if compare_by(a, b, settings, chunk.line_data(), chunk.line_data())
                    > max_allowed_cmp
                {
                    return Err(SortError::Disorder {
                        file: path.to_owned(),
        ...
        pub fn recycle(mut self) -> RecycledChunk {
            let mut recycled_contents = self.with_dependent_mut(|_, contents| {
                ...
                RecycledChunk {
                    ...
                    collation_key_buffer: std::mem::take(
                        &mut contents.line_data.collation_key_buffer,
                    ),
            ...
            match file.read(read_target) {
                Ok(0) => {
                    if read_target.is_empty() {
                        ...
                        let mut sep_iter =
                            memchr_iter(separator, &buffer[newline_search_offset..buffer.len()])
                                .rev();
...
mod ext_sort {
    ...
    mod threaded {
        ...
        use super::super::{
            AtomicOrdering, Compressor, GlobalSettings, Line, OpenFile, Output, SortError,
            SortResult,
            chunks::{self, Chunk, RecycledChunk},
            compare_by, merge,
...
mod merge {
                    ...
                    .map(Box::new(|c: Tmp::Closed| c.reopen())
                        as Box<
                            dyn FnMut(
                                ...
                                -> SortResult<<Tmp::Closed as ClosedTmpFile>::Reopened>,
                        >),
                settings,
                output,
                tmp_dir,
    ...
    impl WriteableTmpFile for WriteableCompressedTmpFile {
        ...
        fn create(
            (file, path): (File, PathBuf),
            compress: Option<&Compressor>,
        ) -> SortResult<Self> {
            let compress = compress
                .expect(
                    "WriteableCompressedTmpFile is only selected when a compressor is configured",
                )
                .clone();
...
fn materialize_stdin(
        ...
        .filter(|file| file.as_os_str() == OsStr::new(STDIN_FILE))
    {
        ...
        if let Some(reader) = stdin.take() {
            std::io::copy(reader, &mut temp).map_err(|error| SortError::ReadFailed {
...
fn emit_debug_warnings(
    ...
    if ignored_opts.len() == 1 {
        show_error!(
            &mut host.stderr,
            "{}",
            format!("option '-{ignored_opts}' is ignored")
        );
    } else if ignored_opts.len() > 1 {
        show_error!(
            &mut host.stderr,
            "{}",
            format!("options '-{ignored_opts}' are ignored")
        );
    }
...
fn uu_app() -> Command {
    Command::new("sort")
        ...
        .arg(make_sort_mode_arg(
            ...
            'R',
            "shuffle in random order".to_owned(),
        ))
        .arg(
...
fn print_sorted<'a, T: Iterator<Item = &'a Line<'a>>>(
    ...
    writer.flush().map_err(|error| SortError::WriteFailed {
        path: output_name,
        error,
    })?;
    Ok(())
