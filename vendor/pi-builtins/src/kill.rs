...
impl builtins::Command for KillCommand {
    ...
    #[allow(
        unknown_lints,
        reason = "unused_async_trait_impl is unknown to the pinned CI nightly"
    )]
    #[allow(
    ...
    async fn execute<SE: brush_core::ShellExtensions>(
        ...
        for arg in &self.args {
            ...
            if !options_done
                && let Some(spec) = arg.strip_prefix('-').filter(|spec| !spec.is_empty())
            {
                ...
                #[cfg(windows)]
                {
                    ...
                    for handle in &handles {
                        let handled = match signal {
                            KillSignal::Probe => {
                                brush_core::processes::process_handle_is_running(handle)
                            }
                            KillSignal::Signal(_) => {
