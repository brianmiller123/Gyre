...
fn touch(
    ...
    let (atime, mtime) = match &opts.source {
        Source::Reference(reference) => {
            ...
            stat(&resolved, !opts.no_deref).map_err(|error| {
                TouchError::ReferenceFileInaccessible(reference.to_owned(), error)
            })?
        }
        Source::Now => {
            let now: FileTime;
    ...
    let (atime, mtime) = if let Some(date) = &opts.date {
        (
            parse_date(
                filetime_to_zoned(&atime, time_zone).ok_or(TouchError::InvalidFiletime(atime))?,
                date,
                time_zone,
            )?,
            parse_date(
                filetime_to_zoned(&mtime, time_zone).ok_or(TouchError::InvalidFiletime(mtime))?,
                date,
                time_zone,
            )?,
        )
        ...
        ChangeTimes::AtimeOnly => (
            ...
            stat(resolved, !opts.no_deref)
                .map_err(|error| {
                    io_context(
                        error,
                        format!("failed to get attributes of {}", path.quote()),
                    )
                })?
                .1,
        ),
...
fn parse_timestamp(s: &str, time_zone: &TimeZone) -> Result<FileTime, TouchError> {
    ...
    let (format, ts) = match s.chars().count() {
        ...
        _ => {
            return Err(TouchError::Message(format!(
                "invalid date format {}",
                s.quote()
            )));
        }
