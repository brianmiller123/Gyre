...
#![allow(
    dead_code,
    reason = "consumed by the feature-gated pgrep/pkill/pidwait front ends"
)]
                ...
                ProcMatchMode::Kill => {
                    ...
                    for process in &processes {
                        ...
                        if options.interactive {
                            ...
                            #[cfg(not(unix))]
                            let response =
                                read_proc_confirmation(&mut stdin, context.cancel_token()).await?;
...
fn parse_proc_match_args(
    ...
    let mut options = ProcMatchOptions {
        delimiter: "\n".to_string(),
        signal: 15,
        ..Default::default()
    };
    ...
    while index < argv.len() {
        ...
        if !options_done && arg.starts_with("--") {
            ...
            match name {
                ...
                "--euid" => {
                    parse_user_list(value.unwrap_or_default(), &mut options.effective_users)?
                }
                ...
                "--terminal" => {
                    parse_terminal_list(value.unwrap_or_default(), &mut options.terminals)?
                }
                "--pidfile" => options
                    .pid_files
                ...
                let value = if takes_value {
                    if short_index + 1 < chars.len() {
                        ...
                        argv.get(index).map(String::as_str).ok_or_else(|| {
                            (2, format!("option '-{option}' requires an argument"))
                        })?
                    }
                } else {
                    ""
        ...
        || !options.terminals.is_empty()
    {
        return Err((
            2,
            "selected process metadata is unavailable on Windows".to_string(),
        ));
    }
    ...
    for file in &options.pid_files {
        let contents = if file == "-" {
            ...
            let mut pidfile = fs::File::open(&path).map_err(|err| {
                (
                    3,
                    format!("cannot read pidfile '{}': {err}", path.display()),
                )
            })?;
            if options.require_lock
                && !pidfile_is_locked(&pidfile).map_err(|err| {
                    (
                        3,
                        format!("cannot inspect pidfile '{}': {err}", path.display()),
                    )
                })?
            {
            ...
            let mut contents = String::new();
            io::Read::read_to_string(&mut pidfile, &mut contents).map_err(|err| {
                (
                    3,
                    format!("cannot read pidfile '{}': {err}", path.display()),
                )
        ...
        && (options.list_name || options.list_full || options.delimiter != "\n")
    {
        return Err((
            2,
            "unsupported output-format option for this command".to_string(),
        ));
    }
...
fn write_proc_match_help(
    ...
    writeln!(
        output,
        "  -P ppid  -p pid  -F pidfile  -O seconds  -r states"
    )?;
    if mode == ProcMatchMode::Kill {
        writeln!(
            output,
            "  -SIGNAL, --signal SIGNAL  choose signal (default TERM)"
        )?;
    }
    #[cfg(target_os = "linux")]
    if mode == ProcMatchMode::Kill {
        writeln!(
            output,
            "  -q value, --queue value  send an integer with sigqueue"
        )?;
    }
    if mode == ProcMatchMode::Grep {
