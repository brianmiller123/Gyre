...
#[command(
    name = "which",
    about = "Locate a command's executable in the shell's PATH"
)]
...
mod tests {
    ...
    fn silent_flag_suppresses_output_and_keeps_exit_status() {
        ...
        assert_eq!(
            run_which(&["-s", "tool"], &path_var, &dir),
            (0, String::new())
        );
        assert_eq!(
            run_which(&["-s", "missing"], &path_var, &dir),
            (1, String::new())
        );
        assert_eq!(
            run_which(&["-as", "tool"], &path_var, &dir),
            (0, String::new())
        );
        assert_eq!(
            run_which(&["-sa", "tool"], &path_var, &dir),
            (0, String::new())
        );
        assert_eq!(
            run_which(&["-s", "tool", "missing"], &path_var, &dir),
            (1, String::new())
        );
    }
    ...
    fn name_with_separator_resolves_against_cwd() {
        ...
        assert_eq!(
            run_which(&["bin/blob"], &path_var, &cwd),
            (1, String::new())
        );
