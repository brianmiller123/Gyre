    ...
    fn apply_modifiers(
        ...
        if effective_width > result.len() {
            ...
            if pad_char == '0' && has_sign {
                ...
                let rest = &result[1..];
                let mut padded =
                    try_alloc_padded(result.len(), padding, effective_width, specifier)?;
                padded.push(sign);
...
fn date_main(host: &mut Host, matches: &ArgMatches) -> Result<(), DateError> {
    ...
    let strptime_value = if strptime_format.is_some() {
            ...
            .is_some_and(|operand| !operand.starts_with('+'))
        {
            ...
            return Err(DateError::new(
                1,
                "'-j -f FORMAT' requires a date operand to parse",
            ));
        }
    } else {
        None
    ...
    let format = if let Some(form) = operands.first() {
        if !form.starts_with('+') {
            ...
            if !matches!(
                date_source,
                DateSource::Human(_) | DateSource::Strptime { .. }
            ) {
...
mod tests {
    ...
    fn bsd_adjustment_rejects_unknown_unit() {
        ...
        assert_eq!(code, 1);
        assert!(
            capture.err().contains("invalid adjustment"),
            "stderr: {}",
            capture.err()
        );
    }
    ...
    fn rewrites_iso_argv_forms_conservatively() {
        ...
        assert_eq!(
            rewrite_date_argv(argv(&["date", "-d", "-I"])),
            argv(&["date", "-d", "-I"])
        );
        // Everything after `--` is an operand.
        assert_eq!(
            rewrite_date_argv(argv(&["date", "--", "-I"])),
            argv(&["date", "--", "-I"])
        );
