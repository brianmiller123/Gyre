...
#![allow(
    dead_code,
    reason = "upstream checksum-common also supports cksum-only formats"
)]
        ...
        (Some(algo @ (AlgoKind::Sha2 | AlgoKind::Sha3)), Some(len)) => {
            ...
            match parsed {
                ...
                _ => {
                    ...
                    Err(failure(ChecksumError::InvalidLengthForSha(
                        algo.to_uppercase().into(),
                    )))
                }
            }
        }
...
fn log_no_properly_formatted(host: &mut Host, filename: impl Display) {
    ...
    let _ = writeln!(
        host.stderr,
        "{name}: {filename}: no properly formatted checksum lines found"
    );
}
