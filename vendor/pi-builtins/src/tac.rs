...
#[allow(
    dead_code,
    reason = "called by the separately feature-gated tail builtin"
)]
...
fn tac_main(matches: &ArgMatches, host: &mut Host) -> Result<(), TacError> {
    ...
    let files: Vec<OsString> = matches.get_many::<OsString>(options::FILE).map_or_else(
        || vec![OsString::from("-")],
...
mod tests {
    ...
    fn invalid_regex_is_fatal_error() {
        ...
        assert_eq!(stdout, "");
        assert!(
            stderr.starts_with("tac: invalid regular expression:"),
            "stderr: {stderr}"
        );
    }
    ...
    fn missing_file_continues_with_next_operand_and_exits_nonzero() {
        ...
        assert_eq!(stdout, "2\n1\n");
        assert!(
            stderr.contains("tac: failed to open 'nope.txt' for reading:"),
            "stderr: {stderr}"
        );
    }
