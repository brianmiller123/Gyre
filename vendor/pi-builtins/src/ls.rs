...
mod colors {
    ...
    impl<'a> StyleManager<'a> {
        ...
        pub(crate) fn apply_style(
            ...
            if !applied_raw_code {
                self.append_style_code_for_style(
                    new_style,
                    &mut style_code,
                    &mut force_suffix_reset,
                );
            }
        ...
        pub(crate) fn apply_missing_target_style(
            &mut self,
            name: OsString,
            wrap: bool,
        ) -> OsString {
        ...
        fn indicator_for_raw_code(&self, path: &PathData) -> Option<Indicator> {
            ...
            let mut entry_exists =
                || -> bool { *existence_cache.get_or_insert_with(|| path.fs_path.exists()) };
        ...
        if let Some(ls_colors) = ls_colors {
            for entry in ls_colors.split(':') {
                ...
                if let Some(indicator) = Indicator::from(key) {
                    ...
                    indicator_codes
                        .insert(indicator, canonicalize_indicator_value(value).into_owned());
                }
            }
        }
...
mod config {
    ...
    fn match_quoting_style_name(
        ...
        let spec = match style {
            ...
            "clocale" => {
                QuotingStyleSpec::with_locale(QuotingStyle::C_DOUBLE, LocaleQuoting::Double)
            }
            _ => return None,
        };
    ...
    impl Config {
        ...
        pub(super) fn from(
            ...
            let (file_size_block_size, block_size) =
                if !opt_si && !opt_hr && !raw_block_size.is_empty() {
            ...
            if needs_color
                && let Some(ls_colors) = host.var("LS_COLORS")
        ...
        if config.format == Format::Long {
            ...
            for item in items {
                ...
                if should_display_leading_info {
                    display_additional_leading_info(
                        item,
                        &padding_collection,
                        config,
                        &mut state.out,
            ...
            match path.fs_path.read_link() {
                Ok(target_path) => {
                    ...
                    if let Some(style_manager) = &mut style_manager {
                        let escaped_target =
                            escape_name_with_locale(target_path.as_os_str(), config);
    ...
    fn calculate_padding_collection(
        ...
        for item in items {
            if config.alloc_size {
                if let Some(md) = item.metadata() {
                    let block_size_len = display_size(get_block_size(md, config), config).len();
                    padding_collections.block_size =
                        block_size_len.max(padding_collections.block_size);
                }
            }
        ...
        entries.sort_unstable_by_key(|p| {
            ...
            !match ft {
                None => {
                    ...
                    get_metadata_with_deref_opt(&p.fs_path, true)
            ...
            .rev()
        {
            ...
            if let Err(err) = fs::read_dir(&e.fs_path) {
                ...
                if state.listed_ancestors.insert(fi) {
                    ...
                    config
                        .runtime
                        .error(LsError::AlreadyListedError(e.path().to_path_buf()));
                }
            }
        }
    }
...
mod integration_tests {
    ...
    fn reads_quoting_style_from_the_shell_environment() {
        ...
        assert!(
            capture
                .err()
                .contains("Ignoring invalid value of environment variable")
        );
    }
    ...
    fn formats_times_in_the_shell_timezone() {
        ...
        assert!(
            capture
                .out()
                .contains("1969-12-31 16:00:00.000000000 -0800")
        );
    }
}
