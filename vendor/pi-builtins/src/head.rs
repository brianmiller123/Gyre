...
mod take {
    ...
    impl TakeAllLinesBuffer {
        ...
        fn write_lines(
            ...
            if max_lines > self.terminated_lines {
                ...
                let index =
                    memchr_iter(separator, self.inner.remaining_buffer()).nth(max_lines - 1);
                assert!(
                    index.is_some(),
    ...
    mod tests {
        ...
        #[allow(
            ...
            reason = "exercises every buffer transition"
        )]
...
fn head_backwards_without_seek_file(
    ...
    match options.mode {
        ...
        Mode::AllButLastLines(n) => {
            read_but_last_n_lines(input, output, n, options.line_ending.into())
        }
        _ => unreachable!(),
    }
}
...
fn head_file(input: &mut File, output: &mut impl Write, options: &HeadOptions) -> io::Result<u64> {
    match options.mode {
        ...
        Mode::AllButLastBytes(_) | Mode::AllButLastLines(_) => {
            head_backwards_file(input, output, options)
        }
    }
}
        ...
        for file in &options.files {
            let result = if file == "-" {
                ...
                match options.mode {
                    ...
                    Mode::AllButLastBytes(n) => read_but_last_n_bytes(&mut input, &mut out, n),
                    Mode::FirstLines(n) => {
                        read_n_lines(&mut input, &mut out, n, options.line_ending.into())
                    }
                    Mode::AllButLastLines(n) => {
                        read_but_last_n_lines(&mut input, &mut out, n, options.line_ending.into())
                    }
                }
            } else {
                ...
                let mut input = match File::open(&resolved) {
                    ...
                    Err(err) => {
                        host.error(
                            format!("cannot open {} for reading: {err}", file.quote()),
                            1,
                        );
                        continue;
                    }
...
mod tests {
    ...
    fn obsolete_num_is_rewritten_at_any_position() {
        assert_eq!(
            rewritten(&["head", "-q", "-5", "f"]),
            ["head", "-q", "-n", "5", "f"]
        );
        assert_eq!(
            rewritten(&["head", "-v", "-20", "f"]),
            ["head", "-v", "-n", "20", "f"]
        );
    ...
    fn option_values_and_post_ddash_operands_are_not_rewritten() {
        assert_eq!(
            rewritten(&["head", "-n", "-5", "f"]),
            ["head", "-n", "-5", "f"]
        );
        assert_eq!(
            rewritten(&["head", "-c", "-5", "f"]),
            ["head", "-c", "-5", "f"]
        );
        assert_eq!(
            rewritten(&["head", "--lines", "-5", "f"]),
            ["head", "--lines", "-5", "f"]
        );
    ...
    fn obsolete_uppercase_suffixes_scale_lines() {
        ...
        assert_eq!(
            options("-1G").unwrap().mode,
            Mode::FirstLines(1024 * 1024 * 1024)
        );
