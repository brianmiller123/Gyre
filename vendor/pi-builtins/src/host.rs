...
fn build_host<SE: ShellExtensions>(
    ...
    for (key, var) in context.shell.env().iter_exported() {
        if var.value().is_set() {
            env.insert(
                key.clone(),
                var.value().to_cow_str(context.shell).into_owned(),
            );
        }
    }
...
#[allow(
    unused_macros,
    reason = "used by utility modules, which are feature-gated"
)]
...
mod testing {
    ...
    impl Host {
        ...
        pub(crate) fn for_test_with_stdin(
            ...
            let host = Self {
                ...
                stderr: StreamWriter::new(OpenFile::Stream(Box::new(MemStream::writer(
                    Arc::clone(&capture.stderr),
                )))),
                name: name.to_string(),
                cwd: cwd.into(),
                env: HashMap::new(),
    ...
    fn resolves_msys_drive_aliases_to_native_drive() {
        ...
        assert_eq!(
            host.resolve("/c/Users/Adam/file.txt"),
            PathBuf::from(r"C:\Users\Adam\file.txt")
        );
    }
    ...
    impl MemStream {
        ...
        fn writer(output: Arc<Mutex<Vec<u8>>>) -> Self {
            Self {
                input: Arc::new(Mutex::new(io::Cursor::new(Vec::new()))),
                output,
            }
        }
    }
    ...
    mod stdout_policy {
        ...
        fn pipe_wrapped_as_file_gets_line_buffering() {
            ...
            assert!(matches!(
                StreamWriter::new(OpenFile::File(file)),
                StreamWriter::Line(_)
            ));
            drop(reader);
        }
...
#[allow(
    unused_imports,
    reason = "used by utility test modules, which are feature-gated"
)]
