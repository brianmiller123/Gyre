    ...
    while index < argv.len() {
        ...
        match arg.as_str() {
            ...
            _ if arg == "--group" || arg.starts_with("--group=") => {
                let value =
                    take_ps_value(argv, &mut index, arg.strip_prefix("--group="), "--group")?;
            ...
            _ if arg == "--format" || arg.starts_with("--format=") => {
                let value =
                    take_ps_value(argv, &mut index, arg.strip_prefix("--format="), "--format")?;
...
fn render_ps_table(rows: &[PsProcessRow], columns: &[PsColumn], no_headers: bool) -> String {
    ...
    let widths: Vec<usize> = columns
        ...
        .enumerate()
        .map(|(index, column)| {
            values.iter().map(|row| row[index].chars().count()).fold(
                column.header.chars().count().max(column.min_width),
                usize::max,
            )
        })
        .collect();
...
fn render_ps_value(
    ...
    match field {
        ...
        PsField::Size => row.virtual_size.zip(ps_page_size()).map_or_else(
            || "?".to_string(),
            |(bytes, page)| bytes.div_ceil(page).to_string(),
        ),
...
mod tests {
    ...
    fn parses_output_field_lists_and_overrides() {
        let argv = vec![
            "-o".to_string(),
            "pid:8=PROCESS,user,args=COMMAND".to_string(),
        ];
    ...
    fn rejects_unknown_output_field() {
        ...
        assert_eq!(
            ...
            (
                1,
                "unknown output format specifier 'definitely_not_a_field'".to_string()
            )
        );
    }
