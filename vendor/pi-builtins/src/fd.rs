...
struct FdCli {
    ...
    #[arg(
        long = "ignore-parent",
        overrides_with = "no_ignore_parent",
        hide = true
    )]
    ignore_parent: bool,
...
impl FdIgnoreMatcher {
    ...
    fn state_for_dir(&mut self, dir: &Path) -> Arc<FdIgnoreState> {
        ...
        if !dir.starts_with(&self.root) {
            return self.states.get(&self.root).map_or_else(
                || {
                    Arc::new(FdIgnoreState {
                        parent: None,
                        matcher: None,
                    })
                },
    ...
    for search_path in &search_paths {
        ...
        for entry in &outcome.entries {
            if cancelled.load(Ordering::Relaxed)
                || max_results.is_some_and(|max| state.matches >= max)
            {
                break;
            }
...
fn try_search_fast(
    ...
    for search_path in search_paths {
        ...
        let status = request.for_each_entry_with_heartbeat(
            ...
            |entry| {
                if cancelled.load(Ordering::Relaxed)
                    || max_results.is_some_and(|max| matches >= max)
                {
...
fn parse_time_filter(value: &str) -> io::Result<SystemTime> {
    ...
    if let Some(duration) = parse_duration(value)? {
        return SystemTime::now().checked_sub(duration).ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
                format!("duration is too large: {value}"),
            )
        });
    }
...
fn parse_owner_filter(value: &str) -> io::Result<OwnerMatcher> {
    ...
    Ok(OwnerMatcher {
        user: parse_owner_side(user)?,
        group: parse_owner_side(group)?,
    })
}
...
fn display_path(config: &SearchConfig, path: &Path) -> String {
    let path = if config.absolute_path || root_was_absolute(path, &config.absolute_roots) {
        ...
        path.strip_prefix(&config.base_dir)
            .unwrap_or(path)
            .to_path_buf()
    };
...
mod tests {
    ...
    fn broken_pipe_on_stdout_is_silent_and_exits_141() {
        ...
        assert!(
            capture.err().is_empty(),
            "stderr must stay clean on a broken pipe"
        );
    }
}
