    ...
    impl Cli {
        ...
        fn long(&mut self, mode: &mut Mode, arg: &str, args: &mut Args) -> Result<(), Error> {
            ...
            match arg {
                ...
                "tab" => self.tab = true,
                "indent" => {
                    self.indent = args.next().and_then(int).ok_or(Error::Int("--indent"))?
                }
    ...
    impl Report {
        fn to_block(&self, idx: &codesnake::LineIndex) -> CodeBlock {
            ...
            Block::new(idx, labels).unwrap().map_code(|c| {
                let c = c.replace('\t', "    ");
                let w = unicode_width::UnicodeWidthStr::width(c.as_str());
...
mod read {
    ...
    fn json_read<'a>(read: impl BufRead + 'a) -> impl Iterator<Item = io::Result<Val>> + 'a {
        ...
        core::iter::from_fn(move || {
            ...
            Some(
                v.map_err(|e| core::mem::take(&mut lexer.error).unwrap_or_else(|| invalid_data(e))),
            )
        })
    }
...
fn real_main(cli: &Cli, host: &mut Host, stdout: &mut dyn Write) -> Result<i32, Error> {
        ...
        for file in &cli.files {
            ...
            if cli.in_place {
                ...
                last = output::with_stdout(stdout, |out| {
                    filter::run(cli, &filter, ctx.clone(), inputs, |v| {
                        output::print(out, cli, &v)
                    })
                })?;
            }
        }
    ...
    if cli.exit_status {
        last.map_or_else(
            || Err(Error::NoOutput),
            |b| if b { Ok(0) } else { Err(Error::FalseOrNull) },
        )
    } else {
        Ok(0)
    }
