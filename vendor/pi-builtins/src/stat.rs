...
for details about the options it supports.";
    ...
    enum StatError {
        ...
        #[cfg_attr(
            not(unix),
            allow(dead_code, reason = "stdin filesystem mode is unix-only")
        )]
        StdinFilesystemMode,
    ...
    impl Stater {
        ...
        fn populate_mount_list() -> Result<Vec<OsString>, StatError> {
            let mut mount_list = read_fs_list()
                .map_err(|e| StatError::CannotReadFilesystem {
                    error: e.to_string(),
                })?
                .iter()
        ...
        #[cfg(unix)]
        fn find_mount_point<P: AsRef<Path>>(&self, p: P, host: &mut Host) -> Option<&OsString> {
            ...
            match *t {
                ...
                Token::Directive {
                    ...
                    let output = match format {
                        ...
                        'w' => OutputType::Str(pretty_time(
                            meta,
                            MetadataTimeField::Birth,
                            self.time_fmt(),
                        )),
                        ...
                        'Y' => {
                            let (sec, nsec) =
                                metadata_get_time(meta, MetadataTimeField::Modification)
                                    .map_or((0, 0), system_time_to_sec);
                            OutputType::Timestamp { sec, nsec }
                        }
    ...
    mod unit_tests {
        ...
        fn normal_format() {
            ...
            assert_eq!(
                &expected,
                &Stater::generate_tokens(s, false, &mut Vec::new()).unwrap()
            );
        }
        ...
        fn printf_format() {
            ...
            assert_eq!(
                &expected,
                &Stater::generate_tokens(s, true, &mut Vec::new()).unwrap()
            );
        }
        ...
        fn test_timestamp_string() {
            ...
            assert_eq!(
                timestamp_string(1712345678, 123_456_789, Precision::Number(0)),
                "1712345678"
            );
            ...
            assert_eq!(
                timestamp_string(1712345678, 5, Precision::Number(3)),
                "1712345678.000"
            );
            assert_eq!(
    ...
    mod win {
        ...
        pub fn statfs(path: &Path) -> Result<StatFs, String> {
            ...
            if unsafe {
                GetVolumePathNameW(file_wide.as_ptr(), root.as_mut_ptr(), root.len() as u32)
            } == 0
            {
            ...
            match *t {
                ...
                Token::Directive {
                    ...
                    let output = match format {
                        ...
                        'd' => {
                            OutputType::Unsigned(ids.as_ref().map_or(0, |ids| ids.volume_serial))
                        }
                        // device number in hex
                        'D' => {
...
mod tests {
    ...
    fn nonexistent_file_reports_cannot_stat() {
        ...
        assert_eq!(stdout, "");
        assert!(
            stderr.starts_with("stat: cannot stat 'missing':"),
    ...
    fn missing_operand_is_error() {
        ...
        assert_eq!(stdout, "");
        assert!(
            stderr.contains("stat: missing operand"),
            "unexpected stderr: {stderr:?}"
        );
        assert!(
            stderr.contains("Try 'stat --help'"),
            "unexpected stderr: {stderr:?}"
        );
    }
    ...
    fn bsd_dash_f_format_is_translated() {
        ...
        assert_eq!(stderr, "");
        assert!(
            stdout.starts_with("data.bin: 12 ("),
            "unexpected stdout: {stdout:?}"
        );
        let epoch = stdout
            .trim_end()
        ...
        assert!(
            ...
            "epoch mtime should be numeric: {stdout:?}"
        );
    }
    ...
    fn bsd_string_form_and_subfield_directives() {
        ...
        assert!(
            perms.chars().all(|c| c.is_digit(8)),
            "octal perms expected: {stdout:?}"
        );
    }
    ...
    fn epoch_time_specifiers_print_integers() {
        ...
        for field in fields {
            assert!(
                ...
                "epoch fields must be integers: {stdout:?}"
            );
        }
    }
    ...
    fn bsd_shell_format_prints_evalable_assignments() {
        ...
        assert!(
            mode.starts_with('0'),
            "octal mode with leading zero: {stdout:?}"
        );
        assert!(
    ...
    fn bsd_verbose_format_prints_linux_like_block() {
        ...
        assert_eq!(stderr, "");
        assert!(
            stdout.starts_with("  File: \"data.bin\"\n"),
            "unexpected stdout: {stdout:?}"
        );
        assert!(
        ...
        assert!(
            ...
            "ctime-style year expected: {access:?}"
        );
    }
    ...
    fn gnu_filesystem_mode_keeps_existing_path_operands() {
        ...
        assert_eq!(stderr, "");
        assert!(
            stdout.contains("Namelen:"),
            "filesystem block expected: {stdout:?}"
        );
    }
