...
fn search_collected_files<M: Matcher, W: Write>(
    ...
    for path in files {
        ...
        let outcome = process_file(
            host, cli, matcher, searcher, &path, display, opts, stats, out,
        )?;
        ...
        match std::fs::metadata(&resolved) {
            Ok(meta) if meta.is_dir() => {
                ...
                for path in files {
                    ...
                    let _ = write_display_bytes(
                        out,
                        display.as_os_str().as_encoded_bytes(),
                        path_separator,
                    );
...
mod tests {
    ...
    fn broken_pipe_on_stdout_is_silent_and_exits_141() {
        ...
        assert_eq!(
            code, SIGPIPE_EXIT_CODE,
            "BrokenPipe must map to 128+SIGPIPE"
        );
        assert!(
    ...
    fn encoding_and_case_insensitive_globs_apply_to_files() {
        ...
        let (code, capture) = run_util::<Rg>(
            &["--encoding=utf-16le", "hit", "utf16.txt"],
            "",
            tree.path(),
        );
        assert_eq!(code, 0, "{}", capture.err());
    ...
    fn null_terminates_paths_without_trailing_newline() {
        ...
        let (_code, capture) = run_util::<Rg>(
            &["--files-without-match", "-0", "nope", "."],
            "",
            tree.path(),
        );
        assert_eq!(capture.out(), "a.txt\0");
