        ...
        async move {
            ...
            for duration in &durations {
                let Some(parsed) = parse_duration(duration) else {
                    let _ = writeln!(
                        context.stderr(),
                        "sleep: invalid time interval '{duration}'"
                    );
...
mod tests {
    ...
    async fn valid_durations_parse_and_short_sleep_completes() {
        ...
        assert_eq!(
            parse_duration("0.000001h"),
            Some(Duration::from_micros(3600))
        );
        assert_eq!(
            parse_duration("0.00000001d"),
            Some(Duration::from_micros(864))
        );
        ...
        let command = SleepCommand {
            durations: vec!["0.001".into(), "0.001s".into()],
        };
    ...
    async fn invalid_duration_reports_original_diagnostic_and_exit_code() {
        ...
        let mut shell = Shell::builder()
            ...
            .expect("test shell should build");
        let command = SleepCommand {
            durations: vec!["not-a-duration".into()],
        };
        let context = ExecutionContext {
            shell: &mut shell,
            command_name: "sleep".into(),
