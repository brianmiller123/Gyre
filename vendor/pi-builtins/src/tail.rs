        ...
        pub fn from(matches: &ArgMatches) -> TailResult<Self> {
            ...
            if let Some(source) = matches.get_one::<String>(options::SLEEP_INT) {
                settings.sleep_sec = parse_time::from_str(source, false).map_err(|_| {
                    TailError::message(format!("invalid number of seconds: '{source}'"))
                })?;
            }
    ...
    mod watch {
        ...
        impl Observer {
                    ...
                    match RecommendedWatcher::new(tx, notify::Config::default()) {
                        ...
                        Err(e) if e.to_string().starts_with("Too many open files") => {
                            ...
                            */
                            ...
                            watcher = Box::new(
                                notify::PollWatcher::new(tx_clone, watcher_config).unwrap(),
                            );
                        }
            ...
            fn handle_event(
                ...
                use notify::event::{
                    CreateKind, DataChange, EventKind, MetadataKind, ModifyKind, RemoveKind,
                    RenameMode,
                };
        ...
        #[allow(
            clippy::cognitive_complexity,
            reason = "preserves upstream follow loop"
        )]
...
mod paths {
    ...
    impl MetadataExtTail for Metadata {
        ...
        fn file_id_eq(
            &self,
            #[cfg(unix)] other: &Metadata,
            #[cfg(not(unix))] _: &Metadata,
        ) -> bool {
            #[cfg(unix)]
            {
...
mod tests {
    ...
    fn obsolete_count_rewritten_at_any_position() {
        assert_eq!(
            rewritten(&["tail", "-20", "f1", "f2"]),
            ["tail", "-n", "20", "f1", "f2"]
        );
        assert_eq!(
            rewritten(&["tail", "-f", "-5", "f"]),
            ["tail", "-f", "-n", "5", "f"]
        );
        assert_eq!(
            rewritten(&["tail", "-5", "-q", "f"]),
            ["tail", "-n", "5", "-q", "f"]
        );
    ...
    fn option_values_and_post_ddash_operands_are_not_rewritten() {
        assert_eq!(
            rewritten(&["tail", "-n", "+5", "f"]),
            ["tail", "-n", "+5", "f"]
        );
        assert_eq!(
            rewritten(&["tail", "-c", "-5", "f"]),
            ["tail", "-c", "-5", "f"]
        );
        assert_eq!(
            rewritten(&["tail", "--lines", "-5", "f"]),
            ["tail", "--lines", "-5", "f"]
        );
