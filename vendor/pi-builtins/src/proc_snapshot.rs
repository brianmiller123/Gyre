    ...
    impl ProcInfo {
        ...
        pub fn age(&self) -> Option<Duration> {
            ...
            Some(Duration::from_secs_f64(
                (uptime - self.stat.start_time as f64 / ticks).max(0.0),
            ))
        }
        ...
        pub fn signal(&self, signal: i32, queue: Option<i32>) -> bool {
            if signal == 0 {
                return read_stat(self.pid)
                    .is_some_and(|stat| stat.start_time == self.stat.start_time);
            }
        ...
        pub fn cpu_time(&self) -> Option<Duration> {
            ...
            Some(Duration::from_secs_f64(
                (self.stat.utime + self.stat.stime) as f64 / ticks as f64,
            ))
        }
...
mod proc_snapshot {
    ...
    impl ProcInfo {
        pub fn all() -> Vec<Self> {
            ...
            let actual = unsafe {
                proc_listallpids(pids.as_mut_ptr(), (pids.len() * size_of::<i32>()) as i32)
            };
            if actual <= 0 {
                return Vec::new();
            }
        ...
        #[allow(
            clippy::unused_self,
            reason = "matches the cross-platform ProcInfo contract"
        )]
...
mod proc_snapshot {
    ...
    impl ProcInfo {
        ...
        pub fn cpu_time(&self) -> Option<Duration> {
            ...
            Some(Duration::from_nanos(
                kernel.saturating_add(user).saturating_mul(100),
            ))
        }
    ...
    fn process_command_line(handle: Handle) -> Option<String> {
        ...
        let units =
            unsafe { std::slice::from_raw_parts(command.buffer, length / size_of::<u16>()) };
...
impl HostProcesses {
    ...
    pub fn resolve_in(all: &[ProcInfo]) -> Self {
        let Ok(self_pid) = i32::try_from(std::process::id()) else {
            return Self {
                pids: smallvec::SmallVec::new(),
                pgids: smallvec::SmallVec::new(),
            };
        };
...
mod tests {
    ...
    fn tree(
        ...
        |pid| {
            nodes
                ...
                .map(|(_, ppid, pgid, start)| ChainNode {
                    ppid: *ppid,
                    pgid: *pgid,
                    start: *start,
                })
    ...
    fn a_recycled_parent_pid_is_not_followed() {
        ...
        assert_eq!(
            host.pids.as_slice(),
            [100],
            "a younger impostor must not join the chain"
        );
        assert!(
            !host.pgids.contains(&9),
            "the impostor's group must not be protected either"
        );
    }
    ...
    fn an_older_parent_is_followed() {
        ...
        assert_eq!(
            host.pids.as_slice(),
            [100, 42, 1],
            "the real chain must be walked to the root"
        );
        assert!(
            host.pgids.contains(&9),
            "an ancestor's group must be protected"
        );
    }
    ...
    fn a_same_tick_parent_is_followed() {
        ...
        assert_eq!(
            host.pids.as_slice(),
            [100, 42],
            "same-tick parent must still be an ancestor"
        );
    }
    ...
    fn a_departed_parent_stops_the_walk() {
        ...
        assert_eq!(
            host.pids.as_slice(),
            [100],
            "an unobserved parent must not join the chain"
        );
    }
    ...
    fn a_departed_parent_leaves_no_stale_pid() {
        ...
        assert_eq!(
            host.pids.as_slice(),
            [self_pid],
            "only observed processes belong in the chain"
        );
    ...
    fn records_our_own_process_group() {
        ...
        assert!(
            host.pgids.contains(&pgid),
            "own process group {pgid} missing"
        );
    }
