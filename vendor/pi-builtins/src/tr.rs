        ...
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            match self {
                ...
                Self::MultipleCharInEquivalence(chars) => {
                    write!(
                        f,
                        "{chars}: equivalence class operand must be a single character"
                    )
                }
            }
        }
        ...
        pub fn flatten(&self) -> Box<dyn Iterator<Item = u8>> {
            match self {
                ...
                Self::Class(class) => match class {
                    ...
                    Class::Punct => {
                        Box::new((33..=47).chain(58..=64).chain(91..=96).chain(123..=126))
                    }
    ...
    impl Sequence {
        pub fn from_str(input: &[u8], stderr: &mut dyn Write) -> Result<Vec<Self>, BadSequence> {
            let parsed = many0(alt((
                ...
                map(
                    Self::parse_backslash_or_char_with_warning,
                    |(s, warning)| (Ok(Self::Char(s)), warning),
                ),
            )))
            .parse(input)
            .map(|(_, r)| r)
