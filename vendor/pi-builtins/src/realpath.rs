...
fn realpath_main(matches: &ArgMatches, host: &mut Host) -> i32 {
    ...
    let (relative_to, relative_base) =
        match prepare_relative_options(matches, host, can_mode, resolve_mode) {
            ...
            Err((path, err)) => {
                host.error(
                    format!("{}: {}", path.maybe_quote(), io_error_message(&err)),
                    1,
                );
...
mod tests {
    ...
    fn zero_flag_terminates_with_nul() {
        ...
        assert_eq!(
            capture.stdout(),
            format!("{}\0", root.join("f").display()).as_bytes()
        );
        assert_eq!(capture.err(), "");
    }
    ...
    fn nonexistent_operand_errors_but_later_operands_still_process() {
        ...
        assert!(
            capture.err().contains("realpath: missing/x"),
            "stderr: {}",
            capture.err()
        );
        assert!(
            capture.err().contains("No such file"),
            "stderr: {}",
            capture.err()
        );
    }
    ...
    fn empty_operand_is_rejected() {
        ...
        assert_eq!(capture.out(), "");
        assert!(
            capture.err().contains("invalid value"),
            "stderr: {}",
            capture.err()
        );
    }
