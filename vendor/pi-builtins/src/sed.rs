...
pub mod command {
    ...
    #[allow(
        dead_code,
        reason = "upstream options retained for command-line compatibility"
    )]
...
pub mod compiler {
                    ...
                    '\\' => {
                        ...
                        match line.current() {
                            ...
                            c @ '0'..='9' => {
                                ...
                                if !literal.is_empty() {
                                    parts.push(ReplacementPart::Literal(std::mem::take(
                                        &mut literal,
                                    )));
                                }
                                if ref_num == 0 {
                ...
                _c @ '1'..='9' => {
                    ...
                    while !line.eol() && line.current().is_ascii_digit() {
                        number = number
                            .checked_mul(10)
                            .and_then(|n| {
                                n.checked_add(line.current().to_digit(10).unwrap() as usize)
                            })
                            .ok_or_else(|| {
                                compilation_error::<()>(
                                    lines,
    ...
    mod tests {
        ...
        fn test_too_many_addresses() {
            ...
            assert!(
                msg.contains(
                    "input.sed:3:1: error: command q expects up to 1 address(es), found 2"
                )
            );
        }
        ...
        fn test_invalid_address_range_posix() {
            ...
            assert!(
                msg.contains(
                    "input.sed:1:1: error: command i expects up to 1 address(es), found 2"
                )
            );
        }
        ...
        fn test_compile_subst_flag_g_and_number_should_fail() {
            ...
            let err = compile_subst_flags(&lines, &mut chars, &mut subst, false, false, None)
                .unwrap_err();
            assert!(
                err.to_string()
        ...
        fn test_compile_read_file_command_rejected_under_sandbox() {
            ...
            let err = compile_read_file_command(&mut lines, &mut chars, &mut cmd, &mut context)
                .unwrap_err();
            assert!(err.to_string().contains(ERR_SANDBOX));
        }
        ...
        fn test_compile_write_file_command_rejected_under_sandbox() {
            ...
            let err = compile_write_file_command(&mut lines, &mut chars, &mut cmd, &mut context)
                .unwrap_err();
            assert!(err.to_string().contains(ERR_SANDBOX));
        }
...
pub mod delimited_parser {
    ...
    mod tests {
        ...
        fn test_validate_quantifier_structure_bre_with_unmatched_brace() {
            let (lines, mut line) = make_providers("{2,3");
            let err = validate_quantifier_structure(&lines, &mut line, '/', RegexMode::Basic)
                .unwrap_err();
            assert!(err.to_string().contains("Unmatched \\{"));
        }
        ...
        fn test_validate_quantifier_structure_ere_with_unmatched_brace() {
            let (lines, mut line) = make_providers("{2,3");
            let err = validate_quantifier_structure(&lines, &mut line, '/', RegexMode::Extended)
                .unwrap_err();
            assert!(err.to_string().contains("Unmatched \\{"));
        }
        ...
        fn next(&mut self) -> Option<Self::Item> {
            match self {
                ...
                CaptureMatches::Fancy(iter) => match iter.next() {
                    ...
                    Some(Err(e)) => Some(Err(SedError::new(
                        2,
...
pub mod processor {
    ...
    use crate::sed::{
        command::{
            Address, AppendElement, Command, CommandData, InputAction, ProcessingContext,
            Transliteration,
        },
        ...
        for ch in line.chars() {
            ...
            let out_str: Cow<str> = match ch {
                ...
                c if c == ' ' || c.is_ascii_graphic() => {
                    Cow::Borrowed(ch.encode_utf8(&mut char_buff))
                }
            ...
            while let Some(command_rc) = current.take() {
                ...
                match command.code {
                    ...
                    'g' => {
                        ...
                        pattern
                            .set_to_string(context.hold.content.clone(), context.hold.has_newline);
                    }
                    'G' => {
                    ...
                    if p.to_string_lossy() == "-" {
                        self.state = State::Active {
                            ...
                            reader: Box::new(BufReader::new(
                                self.stdin.as_ref().expect("stdin stream missing").clone(),
                            )),
                        ...
                        let file = File::open(resolved).map_err_context(|| {
                            format!("error opening script file {}", p.quote())
                        })?;
                        self.state = State::Active {
                            index: next_index,
...
fn sed_main(matches: &ArgMatches, host: &mut Host) -> SedResult<()> {
    ...
    let executable = compiler::compile_with_stdin(
        scripts,
        &mut context,
        host.stdin.file().clone(),
        host.resolve("."),
    )?;
