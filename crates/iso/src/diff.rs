//! Backend 无关的变更捕获。
//!
//! 两条路径，均产出 [`Diff`] = [`FileChange`] 列表：
//!
//! - **Git 模式**：`merged/.git` 存在时，shell `git diff HEAD` + untracked 文件。
//! - **Plain 模式**：无 `.git`，并行遍历两棵树，`(size, mtime)` 短路跳过，
//!   剩余文件用 `similar` crate 产出统一 diff。
//!
//! 移植自 [`pi_iso::diff`](https://github.com/can1357/oh-my-pi)。
...
impl Diff {
    ...
    /// 拼接所有文本文件的 unified diff 文本。跳过二进制文件。
    #[must_use]
...
/// `git diff --no-index` 在文件不同时 exit code = 1，这不是错误。
...
/// 按 `diff --git a/<path> b/<path>` 分割 git diff 输出。
...
// ── plain 模式（mtime-skipped tree walk）──────────────────────────────────────
